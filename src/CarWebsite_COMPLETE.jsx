...
const App = () => {
  ...
  const fetchCars = async () => {
    try {
      ...
      // ✅ המרה לפורמט האתר - השתמש בשדות המובנים!
      const transformedCars = carsData.map(car => ({
        // IDs
        id: car.media_id,
        ad_id: car.ad_id,
        
        // Car Details - ישירות מה-API!
        model: `${car.company_name || ''} ${car.model_name || ''}`.trim() || car.description || 'רכב ללא שם',
        year: parseInt(car.year) || new Date().getFullYear(),
        km: parseInt(car.km) || 0,
        price: parseInt(car.price) || 0,
        
        // Type - נסיק מהדגם
        type: determineCarType(car.model_name, car.company_name, car.description),
        
        // Status
        inStock: car.in_stock !== false, // ברירת מחדל true
        
        // Media
        image: car.s3_url || 'https://images.unsplash.com/photo-1552519507-da3b142c6e3d',
        
        // Descriptions
        description: car.description || car.caption || 'רכב איכותי',
        caption: car.caption,
        ...
        description_timestamp: car.description_timestamp,
  ...
  if (loading) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: '1.5rem', color: '#2563eb', fontWeight: 600 }}>טוען מודעות מהמאגר...</div>
        <div style={{ 
          width: '200px', 
          height: '4px', 
          background: '#e5e7eb', 
